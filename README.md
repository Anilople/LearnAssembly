...
使用`section.段名称.start`来获取某个段的汇编地址, 在汇编时,这个语句代表的值为**段到程序开头的偏移**, 注意这一特性, 在重定位段表的时候, 这里容易出错
...
门(Gate)是另一种形式的描述符,称为门描述符.64位表示.

段描述符用于描述内存段,门描述符则用于描述可执行的代码. 

有好几种类型:

* 调用门(Call Gate): 不同特权级之间的过程调用可以使用. `jmp far`可以将控制通过门转移到比当前特权级高的代码段,但不改变当前特权级别; 但是使用`call far`则会将当前特权级提升到目标代码段的特权级别.
* 中断门/陷阱门: 作为中断处理过程使用
